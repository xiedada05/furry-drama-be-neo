...
// 该文件为 internal/handler/translate.go 的误置副本（错误地放在 internal/repository/
// 目录下但声明 package handler）。正确版本见 internal/handler/translate.go。
// 用 //go:build ignore 排除出默认构建，避免与 repository 包冲突，待后续确认后删除。
package handler
...
func (h *Translate) Translate(c *gin.Context) {
	...
	if !hasKey || jsFalsy(keyVal) || targetLang == "" {
...
// jsFalsy 判断 JS 假值语义（nil/false/0/"" → true）。
